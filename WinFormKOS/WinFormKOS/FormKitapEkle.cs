...
namespace WinFormKOS
{
    public partial class FormKitapEkle : Form
    {
        ...
        bool dolapSiraDoluMu(string dolap, string raf, string sira)
        {
            string query = "SELECT COUNT(*) FROM kitaplar WHERE dolap = @dolap AND raf = @raf AND sira = @sira AND aktif = 1";

            List<SqlParameter> parameters = new List<SqlParameter>()
    {
        new SqlParameter("@dolap", SqlDbType.VarChar) { Value = dolap },
        new SqlParameter("@raf", SqlDbType.VarChar) { Value = raf },
        new SqlParameter("@sira", SqlDbType.VarChar) { Value = sira }
    };

            int count = Convert.ToInt32(IDataBase.executeScalar(query, parameters));
            return count > 0;
        }

            void kitapEkle()
        {
            if (dolapSiraDoluMu(cbbDolap.Text, txtRaf.Text, txtSira.Text))
            {
                MessageBox.Show("Bu raf veya sıra zaten dolu! Lütfen farklı bir yer seçiniz.");
                return;
            }
